...
func GetRunningTotalCostRecords(ctx context.Context, client *spanner.Client, forChart bool) []CostRecord {
	stmt := spanner.Statement{
		SQL: `SELECT date, SUM(cost) AS total_cost FROM jet_tbl GROUP BY date ORDER BY date`,
	}
	iter := client.Single().Query(ctx, stmt)
	...
	for {
		...
		// Always accumulate the cost regardless of whether it's for chart or not
		runningTotalCost += record.Cost
		record.Date = date

		// If it's for chart, append the running total cost record
		if forChart {
			record.Cost = runningTotalCost
			runningTotalCostPerDate = append(runningTotalCostPerDate, record)
		} else {
			// If it's not for chart, append the regular record
			runningTotalCostPerDate = append(runningTotalCostPerDate, record)
		}
	}
